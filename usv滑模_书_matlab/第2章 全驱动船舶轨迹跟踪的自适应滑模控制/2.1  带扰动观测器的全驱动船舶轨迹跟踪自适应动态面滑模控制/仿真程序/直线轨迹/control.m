...
yd=8*t;%u(2)
psid=0.01*t;%u(3)
Vd=[u(4);u(5);u(6)];%滤波器输出
FAI1=[u(7);u(8);u(9)];%虚拟控制率alpha1
T=0.3;
dVd=1/T*(FAI1-Vd);
% dxd=10*cos(0.02*t);
% dyd=10*sin(0.02*t);
...
%估计的扰动观测误差的界 用自适应律求出dDATAG再对其积分得到的DATAG作为控制输入
RAOGUAN=[u(19);u(20);u(21)];
m11=5.3122*10^6;
m22=8.2831*10^6;
m23=0;
m33=3.7454*10^9;
...
S1=ETA-ETAD;
s11=[1 0 0]*S1;%滑模面z1 位置误差向量
s12=[0 1 0]*S1;
s13=[0 0 1]*S1;
S2=VV-Vd;%滑模面z2 横向速度误差向量
s21=[1 0 0]*S2;
s22=[0 1 0]*S2;
s23=[0 0 1]*S2;
epsilon1=0.0005;
...
% K2=diag([10,10,10]);
Lambda=diag([5*10^(-10),5*10^(-10),1*10^(-12)]);
D0=[0.1;0.1;0.1];%扰动观测误差的先验估计
...
dDATAG=CONG*(XI*S2-Lambda*(DATAG-D0));%含修正泄漏项的估计观测误差界的自适应律 式2.1.45
TAO=C*VV+D*VV+M*dVd-K2*S2-XI*DATAG-JT*S1-RAOGUAN;
